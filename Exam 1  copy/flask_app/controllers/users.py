...
def process_reg():
    ...
    if User.validate_registration(request.form):
        ...
        #make sure hashing password comes  after all validations or an error will occur
        print(pw_hash)
        data = {
...
def show_user():
    ...
    pie_info = User.get_pie_with_user_id({'id': session['user_id']})
    return render_template ('/dashboard.html', current_user = current_user, pie_info = pie_info)
