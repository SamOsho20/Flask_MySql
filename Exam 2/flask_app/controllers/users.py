...
def process_reg():
    ...
    if user_in_db:
            ...
            return redirect ('/')
    #this will prevent users with an existing email registering twice!
...
def show_user():
    if 'user_id' not in session:
        return redirect ('/')
    this_user = User.get_one_user({'user_id': session['user_id']})
    return render_template ('dashboard.html', current_user = this_user)
